...
class Classifier(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        if 'emotionnet' in self.finetuning:
            ...
            for m in modules:
                if isinstance(m, nn.Linear) and m.weight.data.size()[0] == 22:
                    ...
                    b1 = torch.FloatTensor(
                        np.array((m.bias.data[1])).reshape(-1))
            ...
            mode = 'ImageNet'
            self.model = model_vgg16(
                pretrained=mode, OF_option=self.OF_option, num_classes=1000)
            modules = self.model.modules()
            for m in modules:
                if isinstance(m,
                              nn.Linear) and m.weight.data.size()[0] == 1000:
                    w1 = m.weight.data[1:2].view(1, -1)
                    b1 = torch.FloatTensor(
                        np.array((m.bias.data[1])).reshape(-1))
            mod = list(self.model.classifier)
            ...
            mode = 'RANDOM'
            self.model = model_vgg16(
                pretrained='', OF_option=self.OF_option, num_classes=1)
